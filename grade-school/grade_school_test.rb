...
class SchoolTest < Minitest::Test
  ...
  # Bonus points
    def test_no_db_mutation_trough_add
      eval = school.add('David', 6)

      if eval.kind_of? Array
        eval << 'External Data'
        refute_equal eval, school.grade(6)
      end
    end

    def test_no_db_mutation_trough_grade
      school.add('Juan', 7)
      eval = school.grade(7)
      eval << 'External Data'
      refute_equal eval, school.grade(7)
    end

    def test_no_db_mutation_trough_to_hash
      school.add('Jonathan', 5)
      eval = school.to_h
      eval[5] << 'External Data'
      refute_equal eval, school.to_h
    end
